...
\label{sec:prp}
The intervention problem is an extension of the plan recognition problem. So we first define the plan recognition problem for the intervening agent.
...
Note that the intervening agent asks this question online, when observations arrive incrementally (not the full set of observations at once).
...
The second method apply machine learning algorithms using features sampled from the plan space of the intervening agent's recognition problem.
At the very latest, the intervening agent must flag actions having post-conditions such that, if added make $\mathrm{u}$ true (e.g.,\textsc{move y2 y3} in Figure~\ref{fig:single}, \textsc{stack B A} in Figure~\ref{fig:multi} (top)). 
...
In this paper, we discuss the design of the decision function $\mathcal{F}$ that flags the latest (i.e., most critical) actions. 
\begin{definition}
A \textnormal{critical action} $a_{crit} \in O$ is an action that occurs in any undesirable plan $\pi_{\mathrm{u}}\in \Pi_{\mathrm{u}}$ and execution of $a_{crit}$ in state $s$ results in a state $s\prime$ such that $s\prime\models \mathrm{u}$.
\end{definition}
\noindent There may be more than one $a_{crit}$ in $O$, so the key decision for the observer is whether to intervene when $a_{crit}$ is observed. For this we use the intervention decision function $\mathcal{F}$
\begin{definition}
An \textnormal{intervention decision function} $\mathcal{F}$ is a function that takes as input, example critical actions in a number of $P_0$ planning problem instances and a set of domain independent features (described next) to produce a learned model that can recognize similar critical actions in an unseen planning instance.
\end{definition}

In this study, we make several assumptions.
\textbf{(Observability)} 
The intervening agent has full observability, while the user can observe everything except $\mathrm{u}$ and the actions (or resulting state) of the competitor, when present.
The observer knows about both $\mathrm{d}$ and $\mathrm{u}$ and helps avoid $\mathrm{u}$.
\textbf{(Plans)} 
...
Throughout execution, the user sticks to his original plan; they do not replan. Execution stops when either $\mathrm{u}$ or $\mathrm{d}$ is reached first.
\textbf{(Competitor)}
...
We define the intervention graph, which models the decision space of the intervening agent.
